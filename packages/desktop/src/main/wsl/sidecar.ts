...
import { type WslCommandLine, resolveWslAo1, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const opencode = await resolveWslAo1(distro)
  ...
  const script = [
    ...
    "export PATH",
    "export WSLENV=",
    "export AO1_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export AO1_CLIENT=desktop",
    `export AO1_SERVER_USERNAME=${shellEscape(username)}`,
    `export AO1_SERVER_PASSWORD=${shellEscape(password)}`,
