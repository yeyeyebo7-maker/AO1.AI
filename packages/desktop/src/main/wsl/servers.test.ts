...
let releaseAo1Resolve: (() => void) | undefined
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.ao1/bin/ao1",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, ao1Checks: {} })
})
...
test("ignores stale background OpenCode checks after removing a WSL server", async () => {
  ...
  await waitFor(() => !!releaseAo1Resolve)
  await controller.removeServer("wsl:Debian")
  releaseAo1Resolve?.()
  await new Promise((resolve) => setTimeout(resolve, 0))

  expect(controller.getState().servers).toEqual([])
  expect(controller.getState().ao1Checks).toEqual({})
})
  ...
  return {
    ...
    resolveAo1: async () => {
      await new Promise<void>((resolve) => {
        releaseAo1Resolve = resolve
      })
      return "/home/me/.ao1/bin/ao1"
    },
  }
}
