...
- `opencode dev web` proxies `https://app.ao1.ai`, so local UI/CSS changes will not show there.
- For local UI changes, run the backend and app dev servers separately.
- Backend (from `packages/ao1`): `bun run --conditions=browser ./src/index.ts serve --port 4096`
