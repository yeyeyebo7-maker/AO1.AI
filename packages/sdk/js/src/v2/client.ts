...
function rewrite(request: Request, values: { directory?: string; workspace?: string }) {
  ...
  for (const [name, key] of [
    ["x-ao1-directory", "directory"],
    ["x-ao1-workspace", "workspace"],
  ] as const) {
    const value = pick(
  ...
  next.headers.delete("x-ao1-directory")
  next.headers.delete("x-ao1-workspace")
  return next
}
...
export function createOpencodeClient(config?: Config & { directory?: string; experimental_workspaceID?: string }) {
  ...
  if (config?.directory) {
    config.headers = {
      ...config.headers,
      "x-ao1-directory": encodeURIComponent(config.directory),
    }
  }
  ...
  if (config?.experimental_workspaceID) {
    config.headers = {
      ...config.headers,
      "x-ao1-workspace": config.experimental_workspaceID,
    }
  }
