...
Collega un terminale a un backend server di OpenCode giÃ  in esecuzione avviato tramite i comandi `serve` o `web`.
...
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Password per l'autenticazione di base (predefinita: `AO1_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `AO1_SERVER_USERNAME` o `opencode`) |
...
Usa `--refresh` per aggiornare l'elenco modelli in cache. Ãˆ utile quando nuovi modelli vengono aggiunti a un provider e vuoi vederli in OpenCode.
...
Esegui opencode in modalitÃ  non interattiva passando un prompt direttamente.
...
Puoi anche collegarti a una istanza `opencode serve` giÃ  in esecuzione per evitare i cold boot dei server MCP ad ogni esecuzione:
...
Avvia un server HTTP che espone accesso API alle funzionalitÃ  di opencode senza la TUI. Imposta `AO1_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
Avvia un server HTTP e apre un browser per accedere a OpenCode tramite interfaccia web. Imposta `AO1_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
OpenCode puÃ² essere configurato tramite variabili d'ambiente.

| Variabile                             | Tipo    | Descrizione                                                 |
| ------------------------------------- | ------- | ----------------------------------------------------------- |
| `AO1_AUTO_SHARE`                 | boolean | Condivide automaticamente le sessioni                       |
| `AO1_GIT_BASH_PATH`              | string  | Percorso all'eseguibile Git Bash su Windows                 |
| `AO1_CONFIG`                     | string  | Percorso al file di configurazione                          |
| `AO1_TUI_CONFIG`                 | string  | Percorso al file di configurazione TUI                      |
| `AO1_CONFIG_DIR`                 | string  | Percorso alla directory di configurazione                   |
| `AO1_CONFIG_CONTENT`             | string  | Contenuto JSON di config inline                             |
| `AO1_DISABLE_AUTOUPDATE`         | boolean | Disabilita i controlli automatici di aggiornamento          |
| `AO1_DISABLE_PRUNE`              | boolean | Disabilita la potatura dei dati vecchi                      |
| `AO1_DISABLE_TERMINAL_TITLE`     | boolean | Disabilita aggiornamenti automatici del titolo terminale    |
| `AO1_PERMISSION`                 | string  | Config permessi JSON inline                                 |
| `AO1_DISABLE_DEFAULT_PLUGINS`    | boolean | Disabilita i plugin di default                              |
| `AO1_DISABLE_LSP_DOWNLOAD`       | boolean | Disabilita download automatico dei server LSP               |
| `AO1_ENABLE_EXPERIMENTAL_MODELS` | boolean | Abilita modelli sperimentali                                |
| `AO1_DISABLE_AUTOCOMPACT`        | boolean | Disabilita compaction automatica del contesto               |
| `AO1_DISABLE_CLAUDE_CODE`        | boolean | Disabilita lettura da `.claude` (prompt + skill)            |
| `AO1_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Disabilita lettura di `~/.claude/CLAUDE.md`                 |
| `AO1_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Disabilita caricamento di `.claude/skills`                  |
| `AO1_DISABLE_MODELS_FETCH`       | boolean | Disabilita fetch dei modelli da fonti remote                |
| `AO1_FAKE_VCS`                   | string  | Provider VCS finto per scopi di test                        |
| `AO1_CLIENT`                     | string  | Identificatore client (default `cli`)                       |
| `AO1_ENABLE_EXA`                 | boolean | Abilita gli strumenti di web search Exa                     |
| `AO1_SERVER_PASSWORD`            | string  | Abilita basic auth per `serve`/`web`                        |
| `AO1_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `opencode`)     |
| `AO1_MODELS_URL`                 | string  | URL personalizzato per recuperare la configurazione modelli |
...
| Variabile                                       | Tipo    | Descrizione                                             |
| ----------------------------------------------- | ------- | ------------------------------------------------------- |
| `AO1_EXPERIMENTAL`                         | boolean | Abilita funzionalitÃ  sperimentali sotto il flag globale |
| `AO1_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Abilita icon discovery                                  |
| `AO1_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Disabilita copy on select nella TUI                     |
| `AO1_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Timeout di default per comandi bash in ms               |
| `AO1_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Massimo token di output per risposte LLM                |
| `AO1_EXPERIMENTAL_FILEWATCHER`             | boolean | Abilita file watcher per l'intera dir                   |
| `AO1_EXPERIMENTAL_OXFMT`                   | boolean | Abilita formatter oxfmt                                 |
| `AO1_EXPERIMENTAL_LSP_TOOL`                | boolean | Abilita strumento LSP sperimentale                      |
| `AO1_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Disabilita file watcher                                 |
| `AO1_EXPERIMENTAL_EXA`                     | boolean | Abilita funzionalitÃ  Exa sperimentali                   |
| `AO1_EXPERIMENTAL_LSP_TY`                  | boolean | Abilita TY LSP per i file python                        |
| `AO1_EXPERIMENTAL_PLAN_MODE`               | boolean | Abilita plan mode                                       |
| `AO1_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Abilita task subagent in background                     |
| `AO1_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | Abilita sistema eventi sperimentale                     |
| `AO1_EXPERIMENTAL_NATIVE_LLM`              | boolean | Abilita percorso nativo di richiesta LLM                |
| `AO1_EXPERIMENTAL_PARALLEL`                | boolean | Abilita esecuzione parallela della ricerca web          |
| `AO1_EXPERIMENTAL_SCOUT`                   | boolean | Abilita subagent Scout                                  |
| `AO1_EXPERIMENTAL_WORKSPACES`              | boolean | Abilita supporto workspace                              |
