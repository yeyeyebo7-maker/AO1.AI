...
OpenCode puÃ² integrarsi con server Language Server Protocol (LSP) per usare la diagnostica come feedback per l'agente.
...
:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `AO1_DISABLE_LSP_DOWNLOAD` a `true`.
:::
...
Quando LSP Ã¨ abilitato e opencode apre un file:
