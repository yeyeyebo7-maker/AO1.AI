...
:::note
Questo strumento e' disponibile solo quando `AO1_EXPERIMENTAL_LSP_TOOL=true` (oppure `AO1_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider OpenCode o quando la variabile d'ambiente `AO1_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).
