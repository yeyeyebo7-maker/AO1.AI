...
:::caution
Se `AO1_SERVER_PASSWORD` non e impostata, il server non sara protetto. Va bene per l'uso locale, ma va impostata per l'accesso in rete.
:::
...
Per proteggere l'accesso, imposta una password usando la variabile d'ambiente `AO1_SERVER_PASSWORD`:

```bash
AO1_SERVER_PASSWORD=secret opencode web
```

Lo username predefinito e `opencode`, ma puo essere cambiato con `AO1_SERVER_USERNAME`.
