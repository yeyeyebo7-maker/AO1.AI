...
3. **Config personalizzata** (variabile d'ambiente `AO1_CONFIG`) - sovrascritture personalizzate
...
6. **Config inline** (variabile d'ambiente `AO1_CONFIG_CONTENT`) - sovrascritture a runtime
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `AO1_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `AO1_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.opencode` e dovrebbe seguire la stessa struttura.
...
Usa `AO1_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, OpenCode prova a usare un modello piÃ¹ economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
