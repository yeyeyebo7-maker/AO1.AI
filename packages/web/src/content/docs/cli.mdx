...
| <nobr><code>{"--session"}</code></nobr>  | `-s`  | Session ID to continue                                                     |
| <nobr><code>{"--fork"}</code></nobr>     |       | Fork the session when continuing (use with `--continue` or `--session`)    |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Basic auth password (defaults to `AO1_SERVER_PASSWORD`)               |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Basic auth username (defaults to `AO1_SERVER_USERNAME` or `opencode`) |
...
This starts an HTTP server that provides API access to opencode functionality without the TUI interface. Set `AO1_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `opencode`).
...
This starts an HTTP server and opens a web browser to access OpenCode through a web interface. Set `AO1_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `opencode`).
...
| ------------------------------------- | ------- | ------------------------------------------------- |
| `AO1_AUTO_SHARE`                 | boolean | Automatically share sessions                      |
| `AO1_GIT_BASH_PATH`              | string  | Path to Git Bash executable on Windows            |
| `AO1_CONFIG`                     | string  | Path to config file                               |
| `AO1_TUI_CONFIG`                 | string  | Path to TUI config file                           |
| `AO1_CONFIG_DIR`                 | string  | Path to config directory                          |
| `AO1_CONFIG_CONTENT`             | string  | Inline json config content                        |
| `AO1_DISABLE_AUTOUPDATE`         | boolean | Disable automatic update checks                   |
| `AO1_DISABLE_PRUNE`              | boolean | Disable pruning of old data                       |
| `AO1_DISABLE_TERMINAL_TITLE`     | boolean | Disable automatic terminal title updates          |
| `AO1_PERMISSION`                 | string  | Inlined json permissions config                   |
| `AO1_DISABLE_DEFAULT_PLUGINS`    | boolean | Disable default plugins                           |
| `AO1_DISABLE_LSP_DOWNLOAD`       | boolean | Disable automatic LSP server downloads            |
| `AO1_ENABLE_EXPERIMENTAL_MODELS` | boolean | Enable experimental models                        |
| `AO1_DISABLE_AUTOCOMPACT`        | boolean | Disable automatic context compaction              |
| `AO1_DISABLE_CLAUDE_CODE`        | boolean | Disable reading from `.claude` (prompt + skills)  |
| `AO1_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Disable reading `~/.claude/CLAUDE.md`             |
| `AO1_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Disable loading `.claude/skills`                  |
| `AO1_DISABLE_MODELS_FETCH`       | boolean | Disable fetching models from remote sources       |
| `AO1_DISABLE_MOUSE`              | boolean | Disable mouse capture in the TUI                  |
| `AO1_FAKE_VCS`                   | string  | Fake VCS provider for testing purposes            |
| `AO1_CLIENT`                     | string  | Client identifier (defaults to `cli`)             |
| `AO1_ENABLE_EXA`                 | boolean | Enable Exa web search tools                       |
| `AO1_SERVER_PASSWORD`            | string  | Enable basic auth for `serve`/`web`               |
| `AO1_SERVER_USERNAME`            | string  | Override basic auth username (default `opencode`) |
| `AO1_MODELS_URL`                 | string  | Custom URL for fetching models configuration      |
...
| Variable                                        | Type    | Description                             |
| ----------------------------------------------- | ------- | --------------------------------------- |
| `AO1_EXPERIMENTAL`                         | boolean | Enable the experimental umbrella flag   |
| `AO1_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Enable icon discovery                   |
| `AO1_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Disable copy on select in TUI           |
| `AO1_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Default timeout for bash commands in ms |
| `AO1_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Max output tokens for LLM responses     |
| `AO1_EXPERIMENTAL_FILEWATCHER`             | boolean | Enable file watcher for entire dir      |
| `AO1_EXPERIMENTAL_OXFMT`                   | boolean | Enable oxfmt formatter                  |
| `AO1_EXPERIMENTAL_LSP_TOOL`                | boolean | Enable experimental LSP tool            |
| `AO1_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Disable file watcher                    |
| `AO1_EXPERIMENTAL_EXA`                     | boolean | Enable experimental Exa features        |
| `AO1_EXPERIMENTAL_LSP_TY`                  | boolean | Enable TY LSP for python files          |
| `AO1_EXPERIMENTAL_PLAN_MODE`               | boolean | Enable plan mode                        |
| `AO1_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Enable background subagent tasks        |
| `AO1_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | Enable experimental event system        |
| `AO1_EXPERIMENTAL_NATIVE_LLM`              | boolean | Enable native LLM request path          |
| `AO1_EXPERIMENTAL_PARALLEL`                | boolean | Enable parallel web search execution    |
| `AO1_EXPERIMENTAL_SCOUT`                   | boolean | Enable Scout subagent                   |
| `AO1_EXPERIMENTAL_WORKSPACES`              | boolean | Enable workspace support                |
