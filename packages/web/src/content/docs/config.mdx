...
3. **Custom config** (`AO1_CONFIG` env var) - custom overrides
4. **Project config** (`opencode.json` in project) - project-specific settings
5. **`.opencode` directories** - agents, commands, plugins
6. **Inline config** (`AO1_CONFIG_CONTENT` env var) - runtime overrides
...
Specify a custom config file path using the `AO1_CONFIG` environment variable.
...
Specify a custom config directory using the `AO1_CONFIG_DIR`
...
Use `AO1_TUI_CONFIG` to point to a custom TUI config file.
