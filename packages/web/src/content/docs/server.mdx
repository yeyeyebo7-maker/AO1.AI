...
Set `AO1_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `opencode`, or set `AO1_SERVER_USERNAME` to override it. This applies to both `opencode serve` and `opencode web`.
