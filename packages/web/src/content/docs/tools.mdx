...
:::note
This tool is only available when `AO1_EXPERIMENTAL_LSP_TOOL=true` (or `AO1_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the OpenCode provider or when the `AO1_ENABLE_EXA` environment variable is set to any truthy value (e.g., `true` or `1`).
