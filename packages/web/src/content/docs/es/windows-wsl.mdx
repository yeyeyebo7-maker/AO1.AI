...
2. **Instala OpenCode en WSL**
   ...
   Cuando WSL estÃ© listo, abre tu terminal de WSL e instala OpenCode con uno de los [mÃ©todos de instalaciÃ³n](/docs/).
...
:::caution
Cuando uses `--hostname 0.0.0.0`, define `AO1_SERVER_PASSWORD` para proteger el servidor.
...
Ejecutar `opencode web` desde WSL garantiza acceso correcto al sistema de archivos e integraciÃ³n con la terminal, manteniÃ©ndolo accesible desde tu navegador de Windows.
...
:::tip
Para una experiencia mÃ¡s fluida, considera clonar o copiar tu repo al sistema de archivos de WSL (por ejemplo en `~/code/`) y ejecutar OpenCode ahÃ­.
:::
...
- Usa la [extensiÃ³n WSL de VS Code](https://code.visualstudio.com/docs/remote/wsl) junto con OpenCode para un flujo de desarrollo integrado
- Tu configuraciÃ³n y tus sesiones de OpenCode se guardan dentro del entorno WSL en `~/.local/share/opencode/`
