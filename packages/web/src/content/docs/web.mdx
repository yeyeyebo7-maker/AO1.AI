...
:::caution
If `AO1_SERVER_PASSWORD` is not set, the server will be unsecured. This is fine for local use but should be set for network access.
:::
...
To protect access, set a password using the `AO1_SERVER_PASSWORD` environment variable:

```bash
AO1_SERVER_PASSWORD=secret opencode web
```

The username defaults to `opencode` but can be changed with `AO1_SERVER_USERNAME`.
