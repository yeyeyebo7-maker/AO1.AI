...
หากคุณต้องการส่งผ่าน environment variables (เช่น `AO1_API_KEY`) คุณต้องระบุใน `env`:
...
```lua {6-8}
{
  acp_providers = {
    ["opencode"] = {
      ...
      args = { "acp" },
      env = {
        AO1_API_KEY = os.getenv("AO1_API_KEY")
      }
    }
  }
}
...
หากคุณต้องส่ง environment variables (เช่น `AO1_API_KEY`) โปรดดู [Configuring Adapters: Environment Variables](https://codecompanion.olimorris.dev/getting-started#setting-an-api-key) ในเอกสารของ CodeCompanion.nvim สำหรับรายละเอียดทั้งหมด
