...
Komanda `opencode serve` pokrece headless HTTP server koji izlaÅ¾e OpenAPI endpoint koji opencode klijent moze koristiti.
...
Postavite `AO1_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `opencode`, ili postavite `AO1_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `opencode serve` i za `opencode web`.
...
Kada pokrenete `opencode`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaÅ¾e OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).
...
opencode server izlaÅ¾e sljedece API-je.
