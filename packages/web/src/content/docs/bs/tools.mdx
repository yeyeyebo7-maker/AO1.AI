...
:::note
Ovaj alat je dostupan samo kada je `AO1_EXPERIMENTAL_LSP_TOOL=true` (ili `AO1_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz OpenCode provajdera ili kada je varijabla `AO1_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).
