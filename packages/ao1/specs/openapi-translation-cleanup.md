...
Trim `packages/ao1/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/ao1/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/ao1`.
- `[x]` `bun typecheck` from `packages/ao1`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/ao1/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/ao1`.
- `[x]` `bun dev generate > /tmp/opencode-openapi.json` from `packages/ao1`.
...
- Edit `packages/ao1/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/opencode-openapi.json` from `packages/ao1`.
...
- Check source schemas in files like `packages/ao1/src/session/schema.ts`, `packages/ao1/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/ao1/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/ao1/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
