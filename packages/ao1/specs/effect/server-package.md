...
- The server still lives in `packages/ao1`.
...
Until enough shared service code lives outside `packages/ao1`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/ao1`

It should not import `packages/ao1` services while `packages/ao1`
...
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/ao1` implementation details.
