...
When `plugin` entries exist in a writable `.opencode` dir or `AO1_CONFIG_DIR`, OpenCode installs `@opencode-ai/plugin` into that dir and writes:
...
- `--pure` / `AO1_PURE` skips external TUI plugins only.
