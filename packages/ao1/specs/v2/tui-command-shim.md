...
Delete `packages/ao1/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/ao1/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/ao1/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/ao1`
- TUI plugin loader tests in `packages/ao1` if runtime plugin API wiring changed
