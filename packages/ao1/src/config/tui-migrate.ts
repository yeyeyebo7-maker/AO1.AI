...
const TUI_SCHEMA_URL = "https://ao1.ai/tui.json"
...
async function opencodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "ao1"),
    ...(await Filesystem.findUp(["ao1.json", "ao1.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "ao1"))
  }
  if (Flag.AO1_CONFIG) files.push(Flag.AO1_CONFIG)
