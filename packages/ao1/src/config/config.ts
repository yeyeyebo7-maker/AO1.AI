...
import { httpClient } from "@ao1-ai/core/effect/layer-node-platform"
...
import type { ConsoleState } from "@ao1-ai/core/v1/config/console-state"
...
import { RemoteAuthError } from "@ao1-ai/core/v1/config/error"
import { ConfigPermissionV1 } from "@ao1-ai/core/v1/config/permission"
...
export class Service extends Context.Service<Service, Interface>()("@ao1/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["ao1.jsonc", "ao1.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.AO1_CONFIG && !Flag.AO1_CONFIG_DIR && !Flag.AO1_CONFIG_CONTENT) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "ao1.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "ao1.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "AO1_CONFIG_CONTENT") return "local"
        ...
        if (Flag.AO1_CONFIG) {
          yield* merge(Flag.AO1_CONFIG, yield* loadFile(Flag.AO1_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.AO1_CONFIG })
        }

        if (!Flag.AO1_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("ao1", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.AO1_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from AO1_CONFIG_DIR", { path: Flag.AO1_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".ao1") || dir === Flag.AO1_CONFIG_DIR) {
            for (const file of ["ao1.json", "ao1.jsonc"]) {
          ...
          // Auto-discovered plugins under `.ao1/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.AO1_CONFIG_CONTENT) {
          const source = "AO1_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.AO1_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from AO1_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["AO1_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("AO1_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.AO1_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.AO1_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("AO1_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.AO1_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.AO1_DISABLE_PRUNE) {
