...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .ao1/ subdirectory too)
  const candidates = [path.join(baseDir, "ao1.json"), path.join(baseDir, "ao1.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".ao1", "ao1.json"), path.join(baseDir, ".ao1", "ao1.jsonc"))
  }
  ...
  // Default to ao1.json if none exist
  return candidates[0]
}
