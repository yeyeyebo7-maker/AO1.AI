...
import type { Hooks } from "@ao1-ai/plugin"
...
export const ProvidersLoginCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.providers.login")(function* (args) {
    ...
    if (provider === "opencode") {
      yield* Prompt.log.info("Create an api key at https://ao1.ai/auth")
    }
    ...
    if (["cloudflare", "cloudflare-ai-gateway"].includes(provider)) {
      yield* Prompt.log.info(
        "Cloudflare AI Gateway can be configured with CLOUDFLARE_GATEWAY_ID, CLOUDFLARE_ACCOUNT_ID, and CLOUDFLARE_API_TOKEN environment variables. Read more: https://ao1.ai/docs/providers/#cloudflare-ai-gateway",
      )
    }
