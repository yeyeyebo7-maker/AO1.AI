This is a plugin to simulate a remote environment locally. Add this to `.opencode/ao1.jsonc`:

```json
  "plugin": ["../packages/ao1/src/control-plane/dev/debug-workspace-plugin.ts"],
```
...
```
./packages/ao1/script/run-workspace-server
```
