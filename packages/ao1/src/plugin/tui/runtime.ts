...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@ao1-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { errorData, errorMessage } from "@ao1-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@ao1-ai/tui/context/theme"
import { Global } from "@ao1-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@ao1-ai/core/util/flock"
import { Flag } from "@ao1-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@ao1-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@ao1-ai/core/v1/config/plugin"
import { createCommandShim } from "@ao1-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@ao1-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".ao1"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".ao1", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".ao1", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.AO1_PURE ? [] : pluginOrigins
    if (Flag.AO1_PURE && pluginOrigins.length) {
    }
