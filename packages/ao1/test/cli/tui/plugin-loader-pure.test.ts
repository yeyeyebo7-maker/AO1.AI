...
`,
  ...
  const pure = process.env.AO1_PURE
  const meta = process.env.AO1_PLUGIN_META_FILE
  process.env.AO1_PURE = "1"
  process.env.AO1_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.AO1_PURE
    } else {
      process.env.AO1_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.AO1_PLUGIN_META_FILE
    } else {
      process.env.AO1_PLUGIN_META_FILE = meta
    }
  }
})
