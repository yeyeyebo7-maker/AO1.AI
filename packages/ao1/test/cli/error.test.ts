...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/ao1.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/ao1.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/ao1.jsonc", dir: ".ao1", suggestion: "ao1" },
        expected:
          'Directory ".ao1" in /tmp/ao1.jsonc is not valid. Rename the directory to "ao1" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        expected: "Configuration is invalid at /tmp/ao1.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats account transport errors clearly", () => {
    const error = new AccountTransportError({
      method: "POST",
      url: "https://console.ao1.ai/auth/device/code",
    })

    const formatted = FormatError(error)

    expect(formatted).toContain("Could not reach POST https://console.ao1.ai/auth/device/code.")
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Or check your config (ao1.json) provider/model names",
    ].join("\n")
