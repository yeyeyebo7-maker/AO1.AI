...
9. Run the focused test file and `bun typecheck` from `packages/ao1`.
...
```bash
git grep -n "Effect.runPromise\|ManagedRuntime\|Promise.withResolvers\|Bun.sleep\|withTestInstance" -- packages/ao1/test
```
