...
import { EventSequenceTable } from "@ao1-ai/core/event/sql"
...
import { WorkspaceTable } from "@ao1-ai/core/control-plane/workspace.sql"
...
const originalEnv = {
  AO1_AUTH_CONTENT: process.env.AO1_AUTH_CONTENT,
  AO1_EXPERIMENTAL_WORKSPACES: process.env.AO1_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.AO1_EXPERIMENTAL_WORKSPACES = "true"
})
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.AO1_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        ...
        expect(JSON.parse(recorded.calls.create[0].env.AO1_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.AO1_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.AO1_EXPERIMENTAL_WORKSPACES).toBe("true")
