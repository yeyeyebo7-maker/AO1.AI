...
const original = {
  ...
  envPassword: process.env.AO1_SERVER_PASSWORD,
  envUsername: process.env.AO1_SERVER_USERNAME,
}
...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) delete process.env.AO1_SERVER_PASSWORD
  else process.env.AO1_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.AO1_SERVER_USERNAME
  else process.env.AO1_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.AO1_SERVER_PASSWORD = auth.password
  Flag.AO1_SERVER_USERNAME = auth.username
  process.env.AO1_SERVER_PASSWORD = auth.password
  process.env.AO1_SERVER_USERNAME = auth.username
...
async function startNoAuthListener() {
  ...
  Flag.AO1_SERVER_USERNAME = auth.username
  delete process.env.AO1_SERVER_PASSWORD
  process.env.AO1_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
...
describe("HttpApi Server.listen", () => {
  ...
  test("plugin client requests reuse the listening server instance", async () => {
    ...
    const previous = process.env.AO1_DISABLE_DEFAULT_PLUGINS
    process.env.AO1_DISABLE_DEFAULT_PLUGINS = "1"
    ...
    try {
      ...
      if (previous === undefined) delete process.env.AO1_DISABLE_DEFAULT_PLUGINS
      else process.env.AO1_DISABLE_DEFAULT_PLUGINS = previous
    }
  })
