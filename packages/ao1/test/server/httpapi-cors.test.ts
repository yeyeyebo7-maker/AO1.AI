    ...
    Effect.gen(function* () {
      const handler = HttpRouter.toWebHandler(
        HttpApiApp.createRoutes().pipe(
          Layer.provide(ConfigProvider.layer(ConfigProvider.fromUnknown({ AO1_SERVER_PASSWORD: "secret" }))),
        ),
        { disableLogger: true },
      ).handler
      const response = yield* Effect.promise(() =>
        handler(
          new Request(new URL("/global/config", "http://localhost"), {
            headers: { origin: "https://app.ao1.ai" },
          }),
          HttpApiApp.context,
        ),
      )
      ...
      expect(response.headers.get("access-control-allow-origin")).toBe("https://app.ao1.ai")
    }),
  )
