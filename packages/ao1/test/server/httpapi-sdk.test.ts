...
import { createOpencodeClient } from "@ao1-ai/sdk/v2"
...
function serverFetch(
  ...
  return HttpServer.HttpServer.use((server) =>
    Effect.sync(() => {
      void serverPath
      Flag.AO1_SERVER_PASSWORD = input?.password
      Flag.AO1_SERVER_USERNAME = input?.username
...
function writeProjectSkill(dir: string) {
  return FSUtil.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".ao1", "skills", "project-rest-skill", "SKILL.md"),
      `---
