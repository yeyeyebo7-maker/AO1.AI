...
function writeProviderAuthPlugin(dir: string) {
  return Effect.gen(function* () {
    ...
    yield* Effect.promise(() => markPluginDependenciesReady(path.join(dir, ".ao1")))

    yield* fs.writeWithDirs(
      path.join(dir, ".ao1", "plugin", "provider-oauth-parity.ts"),
      [
        "export default {",
...
function writeProviderAuthValidationPlugin(dir: string) {
  return Effect.gen(function* () {
    ...
    yield* fs.writeWithDirs(
      path.join(dir, ".ao1", "plugin", "provider-oauth-validation.ts"),
      [
        "export default {",
...
function writeFunctionOptionsPlugin(dir: string) {
  return Effect.gen(function* () {
    ...
    yield* fs.writeWithDirs(
      path.join(dir, ".ao1", "plugin", "provider-function-options.ts"),
      [
        "export default {",
...
function writeProviderModelsMutationPlugin(dir: string) {
  return Effect.gen(function* () {
    ...
    yield* fs.writeWithDirs(
      path.join(dir, ".ao1", "plugin", "provider-models-mutation.ts"),
      [
        "export default {",
