...
// Effect.forkIn loses the request's InstanceRef/WorkspaceRef. It does not â€”
...
describe("HttpApi handler context inheritance", () => {
  ...
  // the request's Context â€” including InstanceRef and WorkspaceRef provided
  // by InstanceContextMiddleware â€” without any explicit re-provide.
  ...
  // Mirrors handlers/session.ts:255 prompt â€” the streaming handler reads
  ...
  it.live("Stream.fromEffect body needs explicit provides â€” inheritance does not carry through", () =>
    Effect.gen(function* () {
