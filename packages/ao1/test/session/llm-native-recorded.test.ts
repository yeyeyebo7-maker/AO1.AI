...
import { LLMEvent, LLMResponse } from "@ao1-ai/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor } from "@ao1-ai/llm/route"
import { Env } from "@/env"
...
const zenURL = (connection: string) => `https://console.ao1.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.AO1_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    canRecord: () => Boolean(envValue("AO1_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("AO1_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    id: "opencode-proxy",
    name: "OpenCode proxy",
    providerID: ProviderV2.ID.ao1,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["opencode", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.AO1_RECORD_CONSOLE_TOKEN && process.env.AO1_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.ao1,
        name: "OpenCode Zen",
        env: ["AO1_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.AO1_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.AO1_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.AO1_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["opencode", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("AO1_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("AO1_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("AO1_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set AO1_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
