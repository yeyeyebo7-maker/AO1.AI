...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "ao1.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "ao1.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "ao1.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "ao1.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["ao1.json", "ao1.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "ao1.json"),
        path.join(project, "ao1.jsonc"),
      ])
    })
  })
