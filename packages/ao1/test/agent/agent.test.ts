...
it.instance("plan agent denies edits except .ao1/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".ao1/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".ao1", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.AO1_TEST_HOME
      process.env.AO1_TEST_HOME = test.directory
