...
export const AO1_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const AO1_MODE_KEY = "ao1.mode"

export const Ao1KeymapProvider = KeymapProvider
export const useAo1Keymap = useKeymap
...
export function createAo1ModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(AO1_MODE_KEY, AO1_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(AO1_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(AO1_MODE_KEY, stack.at(-1)?.mode ?? AO1_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? AO1_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(AO1_MODE_KEY, undefined)
...
export function useAo1ModeStack() {
  return getAo1ModeStack(useAo1Keymap())
}

export function getAo1ModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Ao1 mode stack is not registered for this keymap")
  return value
}
...
export function registerAo1Keymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createAo1ModeStack(keymap)
