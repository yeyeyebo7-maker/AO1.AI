...
// test/preload.ts pins AO1_MODELS_PATH to a fixture so other tests can
...
// bun process.
const ORIGINAL_MODELS_PATH = Flag.AO1_MODELS_PATH
const ORIGINAL_DISABLE_FETCH = Flag.AO1_DISABLE_MODELS_FETCH
beforeAll(() => {
  Flag.AO1_MODELS_PATH = undefined
  Flag.AO1_DISABLE_MODELS_FETCH = true
})
afterAll(() => {
  Flag.AO1_MODELS_PATH = ORIGINAL_MODELS_PATH
  Flag.AO1_DISABLE_MODELS_FETCH = ORIGINAL_DISABLE_FETCH
})
  ...
  it.live("get() recovers from a corrupted cache file by fetching a fresh catalog", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.acquireUseRelease(
        Effect.sync(() => {
          Flag.AO1_DISABLE_MODELS_FETCH = false
        }),
