...
import { RelativePath } from "@ao1-ai/core/schema"
...
describe("Ripgrep", () => {
  ...
  it.live("never includes git metadata", () =>
    Effect.acquireUseRelease(
      ...
      (tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() => fs.mkdir(path.join(tmp.path, ".ao1")))
          yield* Effect.promise(() => fs.writeFile(path.join(tmp.path, ".ao1", "config"), "needle\n"))
          ...
          expect(files.map((item) => item.path)).toContain(RelativePath.make(".ao1/config"))
          ...
          expect(matches.map((item) => item.entry.path)).toContain(RelativePath.make(".ao1/config"))
