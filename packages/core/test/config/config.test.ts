...
import { ConfigProvider } from "@ao1-ai/core/config/provider"
import { ConfigMigrateV1 } from "@ao1-ai/core/v1/config/migrate"
...
describe("Config", () => {
        ...
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              ...
              fs.writeFile(
                path.join(tmp.path, "ao1.json"),
                JSON.stringify({ $schema: "middle", providers: { middle: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "ao1.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "ao1.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
        ...
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              ...
              fs.writeFile(path.join(tmp.path, "ao1.json"), "{ invalid"),
              fs.writeFile(path.join(tmp.path, "ao1.jsonc"), JSON.stringify({ providers: { invalid: true } })),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
  ...
  it.live("loads global, ancestor, and .ao1 configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".ao1"), { recursive: true })
            await fs.mkdir(path.join(directory, ".ao1"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "ao1.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "ao1.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "ao1.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "ao1.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "config.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".ao1", "ao1.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".ao1", "ao1.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".ao1")),
              AbsolutePath.make(path.join(directory, ".ao1")),
            ])
