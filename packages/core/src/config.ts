...
export class Service extends Context.Service<Service, Interface>()("@ao1/v2/Config") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const names = ["config.json", "ao1.json", "ao1.jsonc"]
    ...
    const directories = [
      globalDirectory,
      ...discovered
        .filter((item) => path.basename(item) === ".ao1")
        .toReversed()
        .map((directory) => AbsolutePath.make(directory)),
    ]
    // A config closer to the opened directory should win over one higher up.
    // Search starts nearby, so reverse the results before applying them.
    const directPaths = discovered.filter((item) => path.basename(item) !== ".ao1").toReversed()
    ...
    // global config, project files, then `.ao1` files.
