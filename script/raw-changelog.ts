...
const repo = process.env.GH_REPO ?? "yeyeyebo7-maker/opencode"
...
async function commits(from: string, to: string) {
  ...
  const log =
    await $`git log ${base}..${head} --format=%H -- packages/ao1 packages/sdk packages/plugin packages/desktop packages/app sdks/vscode packages/extensions github`.text()
  ...
  for (const hash of log.split("\n").filter(Boolean)) {
    ...
    for (const file of diff.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/ao1/src/cli/cmd/")) areas.add("tui")
      else if (file.startsWith("packages/ao1/")) areas.add("core")
