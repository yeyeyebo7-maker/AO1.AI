...
- [`help wanted`](https://github.com/yeyeyebo7-maker/opencode/issues?q=is%3Aissue%20state%3Aopen%20label%3Ahelp-wanted)
- [`good first issue`](https://github.com/yeyeyebo7-maker/opencode/issues?q=is%3Aissue%20state%3Aopen%20label%3A%22good%20first%20issue%22)
- [`bug`](https://github.com/yeyeyebo7-maker/opencode/issues?q=is%3Aissue%20state%3Aopen%20label%3Abug)
- [`perf`](https://github.com/yeyeyebo7-maker/opencode/issues?q=is%3Aopen%20is%3Aissue%20label%3A%22perf%22)
...
By default, `bun dev` runs OpenCode in the `packages/ao1` directory. To run it against a different directory or repository:
...
```bash
./packages/ao1/script/build.ts --single
```

Then run it with:

```bash
./packages/ao1/dist/opencode-<platform>/bin/opencode
```

Replace `<platform>` with your platform (e.g., `darwin-arm64`, `linux-x64`).

- Core pieces:
  - `packages/ao1`: OpenCode core business logic & server.
  - `packages/ao1/src/cli/cmd/tui/`: The TUI code, written in SolidJS with [opentui](https://github.com/sst/opentui)
...
> [!NOTE]
> If you make changes to the API or SDK (e.g. `packages/ao1/src/server/server.ts`), run `./script/generate.ts` to regenerate the SDK and related files.
...
- If `spawn` does not work for you, you can debug the server separately:
  - Debug server: `bun run --inspect=ws://localhost:6499/ --cwd packages/ao1 ./src/index.ts serve --port 4096`,
    then attach TUI with `opencode attach http://localhost:4096`
  - Debug TUI: `bun run --inspect=ws://localhost:6499/ --cwd packages/ao1 --conditions=browser ./src/index.ts`
...
- **Denounced users** are explicitly blocked. Issues and pull requests from denounced users are automatically closed. If you have been denounced, you can request to be unvouched by reaching out to a maintainer on [Discord](https://ao1.ai/discord)
