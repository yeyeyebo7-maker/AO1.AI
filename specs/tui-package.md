...
`packages/ao1/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/ao1`,
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/ao1`.
- Do not replace private `packages/ao1` imports with `@opencode-ai/core`
...
```text
packages/ao1/src/cli/cmd/tui
```

Its private dependency on `packages/ao1` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/ao1/src/*`.
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/ao1` initially.
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/ao1`.
...
- The package has no imports from `packages/ao1`, `packages/cli`, or
  `@opencode-ai/core`.
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/ao1`.
- Keep the legacy embedded worker and server startup in `packages/ao1`.
...
- Remove obsolete `@tui/*` path mappings from `packages/ao1`.
...
rg '@opencode-ai/core|packages/ao1|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/ao1/src packages/ao1/test
```
...
- Build the current-platform `packages/ao1` binary.
